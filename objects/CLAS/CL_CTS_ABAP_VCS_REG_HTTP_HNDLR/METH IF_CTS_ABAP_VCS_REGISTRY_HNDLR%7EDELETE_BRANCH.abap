  method if_cts_abap_vcs_registry_hndlr~delete_branch.


    data(action) = if_cts_abap_vcs_registry_hndlr=>co_action_delete_branch.
    ...
    me->logger->log_info( action = action info = |Branch: { name } Force_delete: { force }|  ) ##NO_TEXT.
