  method if_cts_abap_vcs_registry_hndlr~get_branches.


    data(action) = if_cts_abap_vcs_registry_hndlr=>co_action_get_branches.
    ...
    data(http_response) = me->client->get(
        exporting
            iv_path    = |/repository/{ escape( val = id format = cl_abap_format=>e_url ) }/branches|
